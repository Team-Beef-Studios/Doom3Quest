...
public:
	...
	void ReadInternalSavegameVersion(void) { ReadInt(internalSavegameVersion); }

	// if it's 0, this is from a GetBuildNumber() < 1305 savegame otherwise, compare it to idGameLocal::INTERNAL_SAVEGAME_VERSION
	int GetInternalSavegameVersion(void) const { return internalSavegameVersion; }
	// DG end
